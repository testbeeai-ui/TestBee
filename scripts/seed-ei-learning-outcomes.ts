/**
 * Seed Learning Outcomes for Electromagnetic Induction into
 * `learning_outcomes_questions` from tmp-ei-outcomes-parsed.json.
 *
 * Source DOCX: Learning Outccimes Class 12 Physics Electromagnetic Induction.docx
 *
 * Env: NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 * Run: npx tsx --env-file=.env scripts/seed-ei-learning-outcomes.ts
 */
...
const SOURCE = "Learning Outccimes Class 12 Physics Electromagnetic Induction.docx";
...
async function main() {
  ...
  const parsedPath = resolve(process.cwd(), "tmp-ei-outcomes-parsed.json");
  ...
  for (const block of blocks) {
    ...
    for (const row of rows) {
      ...
      updated += 1;
      console.log(
        `✓ ${String(row.level).padEnd(12)} ${row.topic} · ${String(row.subtopic_name).slice(0, 70)} (${payload.length} Qs)`
      );
    }
  }
