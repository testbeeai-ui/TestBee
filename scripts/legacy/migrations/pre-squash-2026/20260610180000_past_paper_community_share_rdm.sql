-- Allow past-paper community shares to claim the same configurable RDM bonus as mock_test posts
-- (one grant per user per attempt_key via mock_community_share_rdm_claims).
...
COMMENT ON FUNCTION public.claim_mock_community_share_rdm IS
  'Authenticated: verify lessons_raw_posts row is mock_test (tag mock_test) or past_paper_result (tag past_paper_share); grant RDM from rdm_config.mock_community_share_rdm once per attempt_key.';
