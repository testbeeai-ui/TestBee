-- Numerals pack: increase daily claim from +15 to +20 RDM (≥60% overall, same IST-day cap).
...
BEGIN
  ...
  v_award := public.award_daily_rdm(v_uid, 'NUMERALS_PACK_COMPLETE', 20);
...
COMMENT ON FUNCTION public.claim_numerals_pack_complete_daily_rdm(text, text, integer, text, text, text) IS
  'All numerals submitted + server-regraded overall ≥60%: +20 RDM once per IST day (NUMERALS_PACK_COMPLETE).';
