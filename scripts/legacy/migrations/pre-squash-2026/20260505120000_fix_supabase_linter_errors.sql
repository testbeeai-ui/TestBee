-- Fix Supabase db lint / advisor issues:
-- 1) toggle_lessons_raw_post_boost: qualify boost_count (RETURNS TABLE column name shadowing)
-- 2) get_adaptive_play_questions: ON CONFLICT uses PK constraint name (avoids RETURNS TABLE "category" ambiguity)
-- 3) daily_gauntlet_attempts: ensure UNIQUE (user_id, gauntlet_date, domain) exists for submit_daily_gauntlet ON CONFLICT
-- 4) classroom_rating_summary: security_invoker view (replace SECURITY DEFINER behavior)

-- ---------------------------------------------------------------------------
-- Gauntlet: guarantee unique index for upsert (idempotent, handles legacy UNIQUE(user_id, gauntlet_date))
-- ---------------------------------------------------------------------------
ALTER TABLE public.daily_gauntlet_attempts
  ADD COLUMN IF NOT EXISTS domain text NOT NULL DEFAULT 'funbrain';

-- Legacy: UNIQUE (user_id, gauntlet_date) only — submit_daily_gauntlet needs (user_id, gauntlet_date, domain).
ALTER TABLE public.daily_gauntlet_attempts
  DROP CONSTRAINT IF EXISTS daily_gauntlet_attempts_user_id_gauntlet_date_key;
ALTER TABLE public.daily_gauntlet_attempts
  DROP CONSTRAINT IF EXISTS daily_gauntlet_attempts_user_id_gauntlet_date_domain_key;

ALTER TABLE public.daily_gauntlet_attempts
  ADD CONSTRAINT daily_gauntlet_attempts_user_id_gauntlet_date_domain_key
...
-- ---------------------------------------------------------------------------
-- get_adaptive_play_questions — same body as 20260430350000, INSERT uses PK for ON CONFLICT
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- classroom_rating_summary — invoker security (Postgres 15+)
-- ---------------------------------------------------------------------------
DROP VIEW IF EXISTS public.classroom_rating_summary;

CREATE VIEW public.classroom_rating_summary
...
GRANT SELECT ON public.classroom_rating_summary TO authenticated;
