-- Hover cards need name/avatar/rdm plus cheap Gyan++ counts in one round trip
-- (one page of authors, not one query per hover).
...
COMMENT ON FUNCTION public.profile_public_previews(uuid[]) IS
  'Batched public hover fields for feed authors. SECURITY DEFINER; no private profile columns.';
