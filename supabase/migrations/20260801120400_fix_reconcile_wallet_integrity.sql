-- Fix inactive-day reconcile after wallet integrity enforcement.
-- The previous function directly updated profiles.rdm, which is blocked by
-- profiles_enforce_rdm_integrity unless the scoped mutation GUC is enabled.

UPDATE public.profiles
SET subscription_started_at = now()
WHERE lower(coalesce(plan_tier, '')) IN ('starter', 'pro')
  ...
  CASE v_plan_tier
    ...
    WHEN 'starter' THEN
      ...
      v_start_date := greatest(
        (coalesce(v_subscription_started_at, now()) + interval '1 day')::date,
        '2026-05-01'::date
      );
    WHEN 'pro' THEN
      SELECT COALESCE(
