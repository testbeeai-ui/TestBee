-- Break posts ↔ teacher_motivation_rdm_grants RLS cycle (grants policy also SELECTs posts).
...
REVOKE ALL ON FUNCTION public.student_has_active_grant_for_assignment(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.student_can_read_post_via_teacher_nudge(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.teacher_owns_motivation_post(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.student_has_active_grant_for_assignment(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.student_can_read_post_via_teacher_nudge(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_owns_motivation_post(uuid) TO authenticated;

DROP POLICY IF EXISTS teacher_motivation_grants_teacher_select ON public.teacher_motivation_rdm_grants;
CREATE POLICY teacher_motivation_grants_teacher_select ON public.teacher_motivation_rdm_grants
  FOR SELECT
  USING (public.teacher_owns_motivation_post(motivation_post_id));
