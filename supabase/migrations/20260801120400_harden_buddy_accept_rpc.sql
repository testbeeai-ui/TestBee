-- Harden the subscription-limit buddy accept overload.
--
-- This RPC is called by the Next.js API with service_role after it resolves the
-- signed-in user and plan limits. Do not expose it directly to browser clients:
-- the acceptor id and effective caps are trusted server inputs.
...
BEGIN
  ...
  -- Serialize cap checks for both users so concurrent accepts cannot exceed plan limits.
  PERFORM 1
  FROM public.profiles
  WHERE id IN (p_acceptor_id, v_invite.inviter_user_id)
  ORDER BY id
  FOR UPDATE;
...
REVOKE ALL ON FUNCTION public.accept_buddy_invite(text, uuid, int, int) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.accept_buddy_invite(text, uuid, int, int) FROM anon;
REVOKE ALL ON FUNCTION public.accept_buddy_invite(text, uuid, int, int) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.accept_buddy_invite(text, uuid, int, int) TO service_role;

COMMENT ON FUNCTION public.accept_buddy_invite(text, uuid, int, int)
  IS 'Service-role-only buddy invite accept RPC; API resolves signed-in acceptor and plan caps before calling.';
