-- Follow-up: 2-arg submit_daily_gauntlet still referenced dropped UNIQUE(user_id, gauntlet_date).
-- get_adaptive_play_questions: PL/pgSQL RETURN QUERY + RETURNS TABLE(column category) — add #variable_conflict use_column.

CREATE OR REPLACE FUNCTION public.submit_daily_gauntlet(p_gauntlet_date date, p_results jsonb)
RETURNS jsonb
LANGUAGE sql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.submit_daily_gauntlet(date, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_daily_gauntlet(date, jsonb) TO anon;
GRANT EXECUTE ON FUNCTION public.submit_daily_gauntlet(date, jsonb) TO authenticated;
