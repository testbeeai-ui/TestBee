...
--
-- Version pinned before post-squash incrementals (2026061811…) so Supabase Preview /
-- fresh DBs create tables before those migrations run.
--
...
--   Legacy applied marker 20260915120000 remains as a no-op stub for history continuity.
--
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'rdm_config_pkey'
  ) THEN
