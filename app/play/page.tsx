...
const FUNBRAIN_DB_ELO_CATEGORIES: FunbrainCategory[] = [
  "puzzles",
  "verbal",
  "quantitative",
  "analytical",
...
const mentalMathDisplayEloKey = (userId: string) =>
  `testbee_play_mental_math_display_elo_${userId}`;
...
const FUNBRAIN_PILLS: {
...
}[] = [
  {
    ...
    label: "Verbal",
    pillClass:
      "border-indigo-400/40 text-indigo-600 bg-indigo-500/10 dark:text-indigo-300 dark:bg-indigo-500/10",
  },
  {
    pillKey: "quantitative",
    id: "quantitative",
    label: "Quantitative",
    pillClass:
      "border-amber-400/50 text-amber-700 bg-amber-500/10 dark:text-amber-300 dark:bg-amber-500/10",
  },
  {
    pillKey: "analytical",
    id: "analytical",
    label: "Analytical",
    pillClass:
      "border-orange-400/50 text-orange-700 bg-orange-500/10 dark:text-orange-300 dark:bg-orange-500/10",
  },
  {
    pillKey: "puzzles",
    id: "puzzles",
    label: "Puzzles",
    pillClass:
      "border-violet-400/45 text-violet-700 bg-violet-500/10 dark:text-violet-300 dark:bg-violet-500/10",
  },
  {
    pillKey: "gk",
    id: "gk",
    label: "GK",
    pillClass:
      "border-emerald-400/45 text-emerald-700 bg-emerald-500/10 dark:text-emerald-300 dark:bg-emerald-500/10",
  },
  {
    pillKey: "mental_math",
    id: "mental_math",
    label: "Mental Math",
    pillClass: "border-red-400/45 text-red-700 bg-red-500/10 dark:text-red-300 dark:bg-red-500/10",
  },
];
...
const PCM_INVESTOR_CHIP: Record<PcmArenaChipId, string> = {
  ...
  math: "border-amber-400/55 text-amber-950 bg-amber-50/40 dark:border-amber-300/50 dark:text-amber-50 dark:bg-amber-950/20",
};
...
const FUNBRAIN_INVESTOR_CHIP: Record<FunbrainCategory, string> = {
  ...
  gk: "border-emerald-500/50 text-emerald-900 bg-emerald-50/35 dark:border-emerald-400/50 dark:text-emerald-100 dark:bg-emerald-950/20",
  mental_math:
...
function PlayPageContent() {
  ...
  const [userStats, setUserStats] = useState<
    { category: string; current_rating: number; win_streak?: number }[]
  >([]);
  ...
  const [gauntletPlayedToday, setGauntletPlayedToday] = useState<{
    academic: boolean;
    funbrain: boolean;
  }>({ academic: false, funbrain: false });
  ...
  const [streakStartUsedByDomain, setStreakStartUsedByDomain] = useState<{
    academic: boolean;
    funbrain: boolean;
  }>({
    academic: false,
  ...
  const fetchUserStats = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select("category, current_rating, win_streak")
      .eq("user_id", user.id);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        storageAcademic =
          localStorage.getItem(playGauntletDayDoneKey(user.id, today, "academic")) === "1";
        storageFunbrain =
          localStorage.getItem(playGauntletDayDoneKey(user.id, today, "funbrain")) === "1";
      } catch {
        /* ignore */
      }
  ...
  const linkStreakTrack = useMemo(() => {
    const raw = searchParams.get("streakTrack");
    const v = (raw ?? "").trim().toLowerCase();
    return isDailyDoseStreakTrackId(v) ? v : null;
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      if (d === "funbrain" || linkStreakTrack) {
        setSelectedDomain("funbrain");
      }
    });
  }, [user?.id, searchParams, linkStreakTrack]);
  ...
  const handleStreakNext = useCallback(() => {
    ...
    if (nextRound >= sessionLen) {
      ...
      setStreakGameoverMentalMath(
        selectedDomain === "funbrain" && streakQuestion.category === "mental_math"
      );
  ...
  const academicClockSec = academicLiveGauntlet
    ...
    : academicLiveStreak
      ? streakSessionLeft
      : null;
  const funbrainClockSec = funbrainLiveGauntlet
    ? gauntletSessionLeft
    : funbrainLiveStreak
      ? streakSessionLeft
      : null;
      ...
      <AppLayout streakTimer={streakTimer}>
        <div className="max-w-5xl mx-auto px-3 py-3 sm:px-4 sm:py-4 2xl:py-6">
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h1 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-[#f0f0ff]">
                    Play
                  </h1>
            ...
            {!playSessionOpen && linkStreakTrack ? (
              <div
                className={cn(
                  "mx-3 sm:mx-[14px] mt-3 rounded-xl border px-3 py-2.5 text-[12px] leading-snug",
                  "border-amber-300/60 bg-amber-500/10 text-amber-950",
                  "dark:border-amber-500/30 dark:bg-amber-500/12 dark:text-amber-100"
                )}
              >
                <span className="font-bold">Class assignment: </span>
                Streak lane —{" "}
                <strong>
                  {DAILYDOSE_STREAK_TRACKS.find((t) => t.id === linkStreakTrack)?.label ??
                    linkStreakTrack}
                </strong>
                ...
                >
                  <div className="flex gap-3 mb-3.5">
                    <div className="flex h-[34px] w-[34px] shrink-0 items-center justify-center rounded-[9px] bg-indigo-500/15 dark:bg-indigo-500/14">
                      <GraduationCap
                        className="h-[19px] w-[19px] text-indigo-600 dark:text-indigo-400"
                        strokeWidth={1.8}
                      />
                    </div>
                    <div className="min-w-0">
                      <h2 className="text-[17px] font-bold leading-tight text-zinc-900 dark:text-[#f0f0ff]">
                        Academic Arena
                      </h2>
                        ...
                        <div
                          className={cn(
                            ...
                            academicClockWarn
                              ...
                              : "text-zinc-900 dark:text-[#f0f0ff]"
                          )}
                        >
                ...
                >
                  <div className="flex gap-3 mb-3.5">
                    <div className="flex h-[34px] w-[34px] shrink-0 items-center justify-center rounded-[9px] bg-orange-500/15 dark:bg-orange-500/12">
                      <Lightbulb
                        className="h-[19px] w-[19px] text-orange-600 dark:text-orange-400"
                        strokeWidth={1.8}
                      />
                    </div>
                    <div className="min-w-0">
                  ...
                  >
                    <Sparkles
                      className="h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-400 mt-0.5"
                      strokeWidth={2}
                    />
                    <span>
                      {isPlayAdmin
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-2 gap-y-2 border-b border-zinc-200/80 dark:border-white/[0.08] pb-2.5 mb-2.5">
                            ...
                            <div className="flex flex-wrap items-center justify-end gap-x-2 gap-y-1 text-[11px] sm:text-xs font-semibold text-zinc-600 dark:text-[#f0f0ff]/70">
                              ...
                              <span className="tabular-nums text-zinc-500 dark:text-[#f0f0ff]/45">
                                {qLabel}
                              </span>
                              <button
                                type="button"
                  ...
                  (() => {
                    ...
                    return (
                      <div className="px-3 pb-3 sm:px-[14px] sm:pb-[14px]">
                        ...
                        >
                          <h2 className="text-xl font-bold text-zinc-900 dark:text-[#f0f0ff] mb-3">
                            {headline}
                          </h2>
                          {!mentalRound && (
                          ...
                          <p className="text-xs text-zinc-500 dark:text-[#f0f0ff]/45 mb-2">
                            ...
                            {streakExitReason === "complete"
                              ...
                              : ""}
                            {mentalRound
                              ? " · Mental Math bank is separate from your Funbrain rating"
                              : ""}
                          </p>
                          {nearMiss && (
                            <p className="text-[11px] text-zinc-500 dark:text-[#f0f0ff]/40 italic mb-3">
                              So close — {nearMiss}
                            </p>
                          )}
                          <p className="text-sm text-zinc-600 dark:text-[#f0f0ff]/65 mb-4">
                            {encouragement}
                          </p>
                  ...
                  <div className="px-3 pb-3 sm:px-[14px] sm:pb-[14px]">
                    <div className="max-w-2xl mx-auto">
                      <Button
                        ...
                        size="sm"
                        onClick={backToDashboard}
                        className="rounded-xl mb-4 -ml-1 h-8 text-xs"
                      >
                        <ArrowLeft className="w-4 h-4 mr-1" /> Back to arena
                      </Button>
                      {gauntletAlreadyPlayed && !gauntletSubmitted && (
                        <p className="text-sm text-zinc-500 dark:text-[#f0f0ff]/50 mb-3">
                          You already played today. See leaderboard below.
                        </p>
                      )}
                      {gauntletSubmitted && (
                        <div className="edu-card mb-4 rounded-2xl border-2 border-emerald-500/25 bg-emerald-500/5 p-4 dark:border-emerald-500/20">
                          <h3 className="text-base font-bold text-zinc-900 dark:text-[#f0f0ff] mb-1">
                            Today&apos;s result
                          </h3>
                      ...
                      <h3 className="text-base font-bold text-zinc-900 dark:text-[#f0f0ff] mb-2">
                        Daily Gauntlet leaderboard
                      </h3>
                      {leaderboardLoading ? (
                            ...
                            return (
                              <li
                                ...
                                className={cn(
                                  ...
                                  !isMe &&
                                    row.rank <= 3 &&
                                    "ring-1 ring-amber-400/25 bg-amber-50/80 dark:bg-amber-950/20"
                                )}
                              >
                                <span
                                  className={cn(
                          ...
                          {gauntletLoadFailed ? (
                            <div className="rounded-xl border border-amber-200/80 bg-amber-50/60 px-3 py-3 dark:border-amber-500/25 dark:bg-amber-950/25">
                              <p className="text-zinc-800 dark:text-[#f0f0ff]/80 mb-2">
                                DailyDose could not load questions (session or network). Try again,
                                or refresh the page.
                              </p>
                              <Button
                                className="rounded-xl font-semibold"
