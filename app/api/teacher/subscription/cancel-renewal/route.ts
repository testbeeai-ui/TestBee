  ...
  try {
    ...
    if (!isTeacherPaidViaRazorpay(details)) {
      return NextResponse.json(
        { error: "Coupon plans end automatically — no renewal to cancel" },
        { status: 400 }
      );
    }
    ...
    const { error: updateError } = await (admin as any)
      ...
      .update({
        payment_card_details: {
          ...(details ?? {}),
          teacher_auto_renew: false,
          teacher_auto_renew_cancelled_at: new Date().toISOString(),
        },
      })
      .eq("id", user.id);
    ...
    console.error("teacher cancel-renewal error", e);
