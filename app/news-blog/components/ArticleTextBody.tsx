  ...
  switch (b.kind) {
    case "h2":
      return (
        <h2
          key={key}
          id={b.id}
          className="mt-10 scroll-mt-24 text-2xl font-semibold tracking-tight text-slate-100"
        >
          {b.text}
        </h2>
      );
    case "h3":
      return (
        <h3
          key={key}
          id={b.id}
          className="mt-8 scroll-mt-24 text-xl font-semibold tracking-tight text-slate-100"
        >
          {b.text}
        </h3>
      );
    ...
    case "blockquote":
      return (
        <blockquote
          ...
          lang="en-IN"
          cite={b.cite}
          className="my-6 rounded-r-md border-l-4 border-emerald-500 bg-slate-900/40 px-5 py-3 italic text-slate-200"
        >
          <p>{b.text}</p>
          {b.cite ? (
    ...
    case "faq":
      return (
        <div
          key={key}
          className="my-5 rounded-md border border-slate-700/60 bg-slate-900/40 p-4"
        >
...
export function ArticleTextBody({ post, showHero = false }: Props) {
  ...
  return (
    <article
      ...
      lang="en-IN"
      id="article-body"
      className="mx-auto w-full max-w-[68ch] text-slate-100"
    >
      <header className="mb-8 border-b border-slate-700/40 pb-6">
        {summary ? (
          <p
            ...
            className="mb-5 text-lg leading-relaxed text-slate-300"
          >
            {summary}
          </p>
        ) : (
          <p className="mb-5 text-sm leading-relaxed text-slate-500">
            No summary on Write text yet — add one when editing; the HTML layout keeps its own
            headline and intro.
          </p>
        )}
        ...
        {showHero && post.heroImageUrl ? (
          <figure className="mt-6">
            ...
            <img
              ...
              decoding="async"
              itemProp="image"
              className="aspect-[1200/630] w-full rounded-lg object-cover ring-1 ring-slate-700/50"
            />
          </figure>
        ) : null}
      </header>
      ...
      {renderToc ? (
        <nav
          ...
          className="mb-8 rounded-lg border border-slate-700/50 bg-slate-900/40 p-4"
        >
      ...
      <div
        ...
        className="space-y-5 text-[1.0625rem] leading-[1.75] text-slate-200"
      >
      ...
      {keywords.length > 0 ? (
        <section
          ...
          className="mt-10 scroll-mt-24 rounded-xl border border-emerald-500/20 bg-emerald-950/15 p-5 sm:p-6"
        >
          <div className="mb-3 flex items-baseline justify-between gap-3">
            <h2
              id="keywords-heading"
              className="text-sm font-semibold uppercase tracking-[0.14em] text-emerald-200"
            >
              Keywords
            </h2>
          ...
          <p className="mb-4 text-xs leading-relaxed text-slate-400">
            Search terms this article is optimised for — helps readers and search engines
          ...
          <ul className="flex flex-wrap gap-2" role="list">
            {keywords.map((k) => (
              <li key={k}>
                <span
                  itemProp="keywords"
                  className="inline-flex items-center rounded-full border border-emerald-500/40 bg-emerald-950/45 px-3 py-1 text-xs font-medium text-emerald-100/95 transition hover:border-emerald-400/60 hover:bg-emerald-900/55"
                >
                  {k}
                </span>
              </li>
