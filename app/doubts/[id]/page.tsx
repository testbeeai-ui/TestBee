...
export default function DoubtDetailPage() {
  ...
  useEffect(() => {
    ...
    supabase.rpc("search_doubt_duplicates", { p_title: doubt.title.trim() }).then(({ data }) => {
  ...
  const handleAccept = async (answerId: string) => {
    ...
    try {
      ...
      if (res?.ok) {
        ...
        toast({
          title: "Could not accept answer",
          description: res?.error ?? "Unable to accept this answer.",
          variant: "destructive",
        });
      }
    } catch (error: unknown) {
      toast({
        title: "Could not accept answer",
        description: getErrorMessage(error, "Network error while accepting answer."),
        variant: "destructive",
      });
    } finally {
      setAcceptingId(null);
  ...
  const handlePostAnswer = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "Network error while posting answer."),
        variant: "destructive",
      });
    } finally {
  ...
  const handleEditDoubtSave = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "Network error while updating question."),
        variant: "destructive",
      });
    } finally {
  ...
  const handleDeleteDoubt = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "Network error while deleting question."),
        variant: "destructive",
      });
    } finally {
  ...
  const handleEditAnswerSave = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "Network error while updating answer."),
        variant: "destructive",
      });
    } finally {
  ...
  const handleDeleteAnswerConfirm = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "Network error while deleting answer."),
        variant: "destructive",
      });
    } finally {
  ...
  const handleReportSubmit = async () => {
    ...
    try {
      ...
      if (error && typeof error === "object" && "code" in error && String(error.code) === "23505") {
        ...
        toast({
          title: "Report failed",
          description: getErrorMessage(error, "Network error while reporting answer."),
          variant: "destructive",
        });
      }
    } finally {
  ...
  const studentAnswerList = answers.filter(
    (a) => a.profiles?.role !== "teacher" && !isAiTutorAnswer(a)
  );
            ...
            {doubt.is_resolved && a.is_accepted && (
              <div className="mt-4 p-4 rounded-xl bg-primary/10 border border-primary/20">
                <p className="text-sm font-medium text-foreground mb-2 flex items-center gap-2">
                  ...
                  Mastered this? Take a 15-minute mock test on {subjectForMock} to see where you
                  stand.
                </p>
                <Button size="sm" className="rounded-xl" asChild>
                  <Link href={`/mock?subject=${encodeURIComponent(subjectForMock)}`}>
                    Start Test
                  </Link>
                </Button>
              </div>
            ...
            <div className="flex items-center justify-between mt-2 flex-wrap gap-2">
              ...
              <div className="flex items-center gap-1">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem
                        ...
                        onClick={() => handleEditAnswerOpen(a)}
                      >
                ...
                {isAuthor && !doubt.is_resolved && !a.is_accepted && (
                  ...
                  >
                    {acceptingId === a.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : null}{" "}
                    Accept
                  </Button>
                )}
              </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div
                      role="heading"
                      aria-level={1}
                      className="text-xl font-bold text-foreground min-w-0 flex-1"
                    >
                  ...
                  <div className="mt-2 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1.5">
                    ...
                    {doubt.is_resolved && (
                      <span className="edu-chip shrink-0 bg-edu-green/10 text-edu-green text-xs">
                        Resolved
                      </span>
                    )}
                  </div>
                  <div
                    id="doubt-body"
                    className="mt-3 text-sm text-muted-foreground max-w-none scroll-mt-24"
                  >
                    {doubt.body ? (
                      <DoubtMarkdown
                        content={stripHtml(doubt.body)}
                  ...
                  <p className="text-xs text-muted-foreground mt-3">
                    Asked{" "}
                    {new Date(doubt.created_at).toLocaleString(undefined, {
                      dateStyle: "medium",
                      timeStyle: "short",
                    })}
                  </p>
            ...
            <Dialog open={editDoubtOpen} onOpenChange={setEditDoubtOpen}>
              <DialogContent className="rounded-2xl max-w-lg">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Update title, details, or subject. You cannot edit after an answer is accepted.
                  </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {DOUBT_FLAIRS.map((flair) => (
                        <Button
                          key={flair}
                          type="button"
                          variant={editDoubtSubject === flair ? "default" : "outline"}
                          size="sm"
                          className="rounded-xl"
                          onClick={() => setEditDoubtSubject(flair)}
                        >
                          {flair}
                        </Button>
                      ))}
                ...
                <DialogFooter>
                  <Button
                    variant="outline"
                    className="rounded-xl"
                    onClick={() => setEditDoubtOpen(false)}
                  >
                    Cancel
                  </Button>
                  <Button
                    className="rounded-xl"
                    onClick={handleEditDoubtSave}
                    disabled={editDoubtSaving || !editDoubtTitle.trim()}
                  >
            ...
            <Dialog open={deleteDoubtConfirmOpen} onOpenChange={setDeleteDoubtConfirmOpen}>
              <DialogContent className="rounded-2xl max-w-sm">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    This cannot be undone. All answers will be removed.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                  <Button
                    variant="outline"
                    className="rounded-xl"
                    onClick={() => setDeleteDoubtConfirmOpen(false)}
                  >
                    Cancel
                  </Button>
                  <Button
                    ...
                    disabled={deleteDoubtLoading}
                  >
                    {deleteDoubtLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : null}{" "}
                    Delete
                  </Button>
                </DialogFooter>
              </DialogContent>
                ...
                <ul className="space-y-2">
                  {similarDoubts.map((s) => (
                    <li key={s.id}>
                      <Link
                        href={`/doubts/${s.id}`}
                        className="text-sm text-primary hover:underline line-clamp-2 block py-0.5"
                      >
                        {s.title}
                      </Link>
                    </li>
                    ...
                    {teacherAnswerList.length === 0 ? (
                      <div className="px-4 sm:px-5 py-4">
                        <p className="text-sm text-muted-foreground">
                          No teacher note yet. When a teacher adds guidance here, it appears above
                          student comments and stays separate from the AI answer.
                        </p>
                      </div>
                    ) : (
            ...
            >
              <DialogContent className="rounded-2xl max-w-sm">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Choose a reason. Your report is anonymous to the answerer.
                  </DialogDescription>
                </DialogHeader>
                ...
                <DialogFooter>
                  <Button
                    variant="outline"
                    className="rounded-xl"
                    onClick={() => setReportAnswerId(null)}
                  >
                    Cancel
                  </Button>
                  <Button
                  ...
                  >
                    {reportSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : null} Submit
                    report
                  </Button>
                </DialogFooter>
              </DialogContent>
            ...
            <Dialog open={!!editAnswerId} onOpenChange={(open) => !open && setEditAnswerId(null)}>
              <DialogContent className="rounded-2xl max-w-lg">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    You cannot edit after your answer is accepted.
                  </DialogDescription>
                </DialogHeader>
                <div className="py-2">
                ...
                <DialogFooter>
                  ...
                  <Button
                    className="rounded-xl"
                    onClick={handleEditAnswerSave}
                    disabled={editAnswerSaving || !editAnswerBody.trim()}
                  >
            ...
            >
              <DialogContent className="rounded-2xl max-w-sm">
                ...
                <DialogFooter>
                  ...
                  >
                    {deleteAnswerLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : null}{" "}
                    Delete
                  </Button>
                </DialogFooter>
              </DialogContent>
