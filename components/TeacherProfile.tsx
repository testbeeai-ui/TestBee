        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          >
            ...
            <div className="text-center sm:text-left flex-1">
              ...
              {/* Google sign-in vs Google Calendar OAuth (Meet / scheduling) */}
              <div className="mt-4 space-y-2">
                {profile.signup_google ? (
                  <p className="text-xs text-muted-foreground max-w-md">
                    You sign in with Google. That is separate from{" "}
                    <span className="font-semibold text-foreground">Google Calendar access</span>{" "}
                    (Meet links and calendar sync require an extra consent screen).
                  </p>
                ) : null}
                {profile.google_connected ? (
                  <p className="text-sm font-bold text-edu-green flex items-center gap-1.5">
                    ...
                    Google Calendar linked — Meet / scheduling enabled
                  </p>
                ) : (
                  <div className="flex flex-col items-center gap-2 sm:items-start">
                    <Button
                      variant="outline"
                      size="sm"
                      disabled={calendarLinkBusy}
                      onClick={async () => {
                        try {
                          setCalendarLinkBusy(true);
                          const r = await redirectToGoogleCalendarConsent();
                          if (r.mode === "popup") {
                            if (r.connected) {
                              toast({ title: "Google Calendar linked" });
                              await refreshProfile();
                            } else if (r.reason && r.reason !== "closed") {
                              toast({
                                title: "Could not link Google Calendar",
                          ...
                          toast({
                            title: "Could not open Google Calendar consent",
                            description:
                              err instanceof Error ? err.message : "Please try again in a moment.",
                            variant: "destructive",
                          });
                        } finally {
                          setCalendarLinkBusy(false);
                        }
                      }}
                      className="rounded-xl font-bold gap-2 mt-1"
                    >
          ...
          >
            ...
            {editing ? (
              <div className="space-y-5">
                <div>
                  <label className="text-sm font-extrabold text-foreground mb-1.5 block">
                    Name
                  </label>
                  <Input
                    value={name}
                ...
                <div>
                  <label className="text-sm font-extrabold text-foreground mb-2 block">
                    Teaching levels
                  </label>
