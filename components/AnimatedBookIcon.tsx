...
const generatePagePath = (t: number, isCover: boolean, rightY: number, leftY: number) => {
  const steps = 12;
  const width = 180;
  const zTop = -120;
  const zBottom = 120;
  ...
  for (let i = 1; i <= steps; i++) d += ` L ${topPoints[i].x} ${topPoints[i].y}`;
  for (let i = steps; i >= 0; i--) d += ` L ${bottomPoints[i].x} ${bottomPoints[i].y}`;
  d += " Z";
  return d;
};
...
function AnimatedPage({
  ...
  const d = useTransform(progress, (t: number) => generatePagePath(t, isCover, rightY, leftY));
...
function CoverPage({
  ...
  const d = useTransform(progress, (t: number) => generatePagePath(t, true, rightY, leftY));
  const opacity = useTransform(progress, (t: number) => {
    if (side === "right") return t < 0.5 ? 1 : 0;
    if (side === "left") return t >= 0.5 ? 1 : 0;
    return 1;
  ...
  return (
    <motion.g style={{ opacity }}>
      <motion.path d={d} fill={fill} stroke={stroke} strokeWidth={2} strokeLinejoin="round" />
      <motion.path d={d} fill="black" style={{ opacity: shadowOpacity }} />
    </motion.g>
  );
}
...
export default function AnimatedBookIcon({
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          <linearGradient id="abCoverGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#8B5CF6" />
            <stop offset="100%" stopColor="#5B21B6" />
          </linearGradient>
          <linearGradient id="abCoverGradDark" x1="0%" y1="0%" x2="100%" y2="100%">
