...
export default function AssignmentCompletionEscrowConfirm({
  ...
  const publishFee = showSubtopicUnlock ? 0 : Math.max(0, Math.round(publishFeeRdm));
  const grandTotal = unlock.total + escrow.total + publishFee;
  const isOverage = publishFeeKind === "overage";

  const title =
    grandTotal > 0
      ? `Publish for ${formatRdmDeduction(grandTotal)}?`
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-4">
      ...
      >
        ...
        {isOverage && tier === "pro" && publishFee > 0 ? (
          <p className="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-sm leading-snug text-amber-100/95">
            {teacherProOverageExplain("assignment", monthlyCap, publishFee)}
          </p>
        ) : null}
        ...
        {showSubtopicUnlock && unlock.total > 0 ? (
          <div className="mt-4 space-y-2 rounded-xl border border-sky-500/25 bg-sky-500/5 p-3 text-sm text-slate-200">
            ...
            <div className="flex justify-between gap-3">
              ...
              <span className="font-semibold text-sky-200">{formatRdmDeduction(unlock.perStudent)}</span>
            </div>
            <div className="flex justify-between gap-3">
            ...
            <div className="flex justify-between gap-3 border-t border-white/10 pt-2">
              ...
              <span className="font-bold text-sky-200">{formatRdmDeduction(unlock.total)}</span>
            </div>
          </div>
        ) : null}
        ...
        {escrow.total > 0 ? (
          <div className="mt-3 space-y-2 rounded-xl border border-emerald-500/25 bg-emerald-500/5 p-3 text-sm text-slate-200">
            ...
            <div className="flex justify-between gap-3">
              <span className="text-slate-400">Per student (they earn)</span>
            ...
            <div className="flex justify-between gap-3 border-t border-white/10 pt-2">
              ...
              <span className="font-bold text-amber-300">{formatRdmDeduction(escrow.total)}</span>
            </div>
          </div>
        ) : null}

        {!showSubtopicUnlock && publishFee > 0 ? (
          <div className="mt-3 flex justify-between gap-3 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-300">
            <span>{assignmentPublishFeeLine({ kind: publishFeeKind, amount: publishFee, tier })}</span>
            <span className="font-semibold text-slate-100">{formatRdmDeduction(publishFee)}</span>
          </div>
        ) : null}

        {!showSubtopicUnlock && publishFeeKind === "none" && tier === "pro" ? (
          <div className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-3 py-2 text-xs text-emerald-200/90">
            Included on Pro — no flat publish fee within your monthly quota.
          </div>
        ) : null}
        ...
        {grandTotal > 0 ? (
          <div className="mt-3 flex justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm">
            ...
            <span className="font-bold text-amber-200">{formatRdmDeduction(grandTotal)}</span>
          </div>
        ) : null}
        ...
        {escrow.total > 0 ? (
          dueDateLabel ? (
            <p className="mt-3 text-xs text-slate-500">
              ...
              receive rewards. Unused reward slots return to you after the deadline.
            </p>
          ) : (
            <p className="mt-3 text-xs text-slate-500">
              Unused completion rewards return to you if students do not finish.
            </p>
          )
        ) : null}
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          ...
          >
            {busy
              ? "Creating…"
              : grandTotal > 0
                ? `Yes, publish (${formatRdmDeduction(grandTotal)})`
                : "Yes, publish"}
          </button>
        </div>
      </div>
