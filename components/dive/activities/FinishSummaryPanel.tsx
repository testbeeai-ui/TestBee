...
export default function FinishSummaryPanel({
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      <span className="mb-2.5 inline-flex items-center gap-1.5 rounded-full border border-primary/25 bg-primary/10 px-3 py-1 text-[10.5px] font-extrabold uppercase tracking-wider text-primary">
        <i className="ti ti-trophy text-xs" aria-hidden="true" />
        Assessment Result
      </span>

      <h3 className="mb-1 text-xl font-extrabold text-foreground">{title}</h3>
      <div className={`mb-5 inline-flex items-center gap-1.5 rounded-full border px-3 py-0.5 text-xs font-semibold ${statusBadge.bg}`}>
        <span>{statusBadge.emoji}</span>
        <span>{statusBadge.label}</span>
      </div>

      <div
        className={`relative mb-6 flex h-32 w-32 items-center justify-center rounded-full border bg-gradient-to-b ${ringGlow}`}
      >
        <div className="flex flex-col items-center justify-center">
          <span className="text-4xl font-black tracking-tight">{scorePct}%</span>
      ...
      <div className="mb-6 flex items-center gap-2 rounded-xl border border-border/50 bg-muted/20 px-3.5 py-2.5 text-xs text-muted-foreground">
      ...
      <button
        type="button"
        className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-2.5 text-sm font-bold text-primary-foreground shadow-lg transition hover:scale-[1.02] hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 active:scale-[0.98]"
        onClick={onContinue}
      >
