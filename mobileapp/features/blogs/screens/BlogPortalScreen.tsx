...
import { BLOG_SECTION_LABELS, type NewsBlogPost } from "@/core/domain/newsBlog";
import { useBlogPosts } from "@/features/news/hooks/useNewsBlogPosts";
import { Screen } from "@/shared/components/ui/Screen";
import { colors, spacing } from "@/shared/constants/theme";
import { formatTimeAgo } from "@/core/domain/doubts";

export function BlogPortalScreen() {
    ...
    <Screen padded={false}>
        ...
        renderItem={({ item }) => (
          ...
          >
            <Text style={[styles.chipText, section === item && styles.chipTextActive]}>
              {item === "all" ? "All" : (BLOG_SECTION_LABELS[item] ?? item)}
            </Text>
          </Pressable>
        )}
      />
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable onPress={() => openPost(item)} style={styles.card}>
              <Text style={styles.section}>
                {BLOG_SECTION_LABELS[item.section] ?? item.section}
              </Text>
          ...
          ListEmptyComponent={<Text style={styles.empty}>No blog posts yet.</Text>}
        />
      )}
    </Screen>
  );
