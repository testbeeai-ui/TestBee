...
## Decisions Log
- 2026-06-04: Inactive-day reconcile wallet fix — new migration `20260801120400_fix_reconcile_wallet_integrity.sql` backfills paid `subscription_started_at` and wraps penalty RDM updates with `app.allow_profile_rdm_mutation`; testing `set-plan` now preserves/sets paid start dates.
- 2026-06-04: Free trial activation is idempotent server-side; repeated or racing POSTs no longer reset checklist claim state or double-credit welcome RDM (`activate-trial` route + route tests).
